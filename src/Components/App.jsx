...
function App() {
  ...
  const handleExpensesBtn = () => {
    ...
    if (!expenseData.name || !expenseData.description || !expenseData.date || expenseData.expenses === 0) {
  ...
  return (
    <>
      <div className="grid grid-cols-6 grid-rows-2 h-[51.6rem] gap-0 bg-gray-900">
            ...
            <div className="flex text-center justify-center mt-2">
              <div className="text-wrap">
                ...
                <input
                  ...
                  onChange={(e) => setExpenseData({ ...expenseData, expenses: e.target.value })}
                  className="mb-2 w-[15rem] text-black font-semibold text-center rounded-md p-2 outline-none"
                />
                <button
                  type="button"
            ...
            <tbody className="border-b border-x-2 text-[20px]">
              {expensesList.map((expense, index) => (
                <tr key={index} className="border-b border-x-2">
                 <td className="border border-white text-white"> {expense.name}</td>
                  <td className="border border-white text-white">{expense.description}</td>
                  <td className="border border-white text-white">{expense.date}</td>
                  <td className="border border-white text-white">₱{parseFloat(expense.expenses).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
